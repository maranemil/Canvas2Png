// noinspection JSUnusedGlobalSymbols,JSCheckFunctionSignatures,JSUnresolvedVariable,SpellCheckingInspection,GrazieInspection
...
DSP.deinterleave = function (buffer) {
    ...
    const mix = new Float32Array(buffer.length / 2);

    let i = 0, len = buffer.length / 2;
    for (; i < len; i++) {
        left[i] = buffer[2 * i];
...
ADSR.prototype.processSample = function (sample) {
    ...
    if (this.samplesProcessed <= this.attack) {
        amplitude = (1) * ((this.samplesProcessed) / (this.attack - 0));
...
DSP.mag2db = function (buffer) {
    ...
    const result = Float32Array(buffer.length);
    for (let i = 0; i < buffer.length; i++) {
...
DSP.freqz = function (b, a, w) {
    ...
    const result = Float32Array(w.length);
...
MultiDelay.prototype.process = function (samples) {
    ...
    let outputSamples = new Float32Array(samples.length);
...
Reverb.prototype.process = function (interleavedSamples) {
    let i;
// NB. Make a copy to put in the output samples to return.
    let outputSamples = new Float32Array(interleavedSamples.length);
    ...
    let singleDelaySamples = new Float32Array(outputSamples.length);
    for (i = 0; i < this.NR_OF_SINGLEDELAYS; i++) {
...
function mixSampleBuffers(sampleBuffer1, sampleBuffer2, negate, volumeCorrection) {
    const outputSamples = new Float32Array(sampleBuffer1);
